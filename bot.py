import requests
from bs4 import BeautifulSoup
import time
import os
from datetime import datetime
import hashlib

print("🚀 Bot gestartet")

URL = "https://www.mainz-tauschen-verschenken.de/"

TOKEN = "8704089663:AAGwNXYGJu20o2jQs-tGLyIxZqmPG2jxMx8"
CHAT_ID = "7520498009"

HEADERS = {"User-Agent": "Mozilla/5.0"}

SEEN_FILE = "seen.txt"


def load_seen():
    if not os.path.exists(SEEN_FILE):
        print("📂 Keine seen.txt gefunden → starte leer")
        return set()
    with open(SEEN_FILE, "r") as f:
        seen = set(line.strip() for line in f)
        print(f"📂 {len(seen)} gespeicherte Anzeigen geladen")
        return seen


def save_seen(seen):
    with open(SEEN_FILE, "w") as f:
        for item in seen:
            f.write(item + "\n")
    print(f"💾 {len(seen)} Anzeigen gespeichert")


def make_id(text):
    return hashlib.md5(text.encode("utf-8")).hexdigest()


def send(msg):
    print("📤 Sende Nachricht an Telegram")
    url = f"https://api.telegram.org/bot{TOKEN}/sendMessage"
    requests.post(url, data={"chat_id": CHAT_ID, "text": msg})


def check(seen):
    print("🌐 Hole Webseite...")
    r = requests.get(URL, headers=HEADERS, timeout=10)
    print("✅ Seite geladen:", r.status_code)

    soup = BeautifulSoup(r.text, "html.parser")

    ads = soup.select("article")
    print(f"📦 {len(ads)} Anzeigen gefunden")

    new_seen = set(seen)
    new_count = 0

    for ad in ads:
        text = ad.get_text(strip=True)
        ad_id = make_id(text)

        if ad_id not in seen:
            print("🆕 Neue Anzeige gefunden")
            new_seen.add(ad_id)
            new_count += 1
            send("🆕 Neue Anzeige:\n" + text)

    print(f"➡️ {new_count} neue Anzeigen gesendet")
    return new_seen


def get_sleep_time():
    hour = datetime.now().hour
    if hour >= 23 or hour < 6:
        print("😴 Nachtmodus (30 Minuten Pause)")
        return 1800
    else:
        print("⚡ Aktivmodus (3 Minuten Pause)")
        return 180


seen = load_seen()

while True:
    try:
        print("\n🔁 Neuer Durchlauf:", datetime.now())
        seen = check(seen)
        save_seen(seen)
        print("✅ Check fertig")

    except Exception as e:
        print("❌ Fehler:", e)

    sleep_time = get_sleep_time()
    print(f"⏳ Schlafe {sleep_time} Sekunden...\n")
    time.sleep(sleep_time)

  
